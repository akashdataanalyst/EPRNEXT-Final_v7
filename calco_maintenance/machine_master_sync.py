...
def sync_machine_master_after_migrate():
    if not frappe.db.exists("DocType", "Maintenance Equipment"):
        return

    if not frappe.db.has_column("Maintenance Equipment", "machine_code"):
        return

    sync_machine_master_fields()

    if frappe.db.exists("DocType", "Maintenance Ticket") and frappe.db.has_column("Maintenance Ticket", "machine_name"):
...
def preserve_existing_ticket_machine_values():
    ...
    for row in rows:
        ...
        if frappe.db.has_column("Maintenance Ticket", "equipment") and cstr(row.equipment) != machine_docname:
