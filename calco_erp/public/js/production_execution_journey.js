...
(function () {
  const TRACKER_FIELDS = ["journey_tracker_html", "custom_production_journey_html"];
  const STYLE_ID = "calco-production-execution-style";
  const SUPPORTED_DOCTYPES = ["Production Requirement", "Production Job Card", "Work Order"];
  ...
  function getWrapper(frm) {
    return TRACKER_FIELDS.reduce((wrapper, fieldname) => {
      if (wrapper) {
        return wrapper;
      }
  ...
  function addWorkOrderLifecycleButtons(frm) {
    if (frm.doctype !== "Work Order" || frm.is_new() || frm.doc.docstatus === 2) {
      return;
    }

    frm.add_custom_button(__("Create Initial QC"), async () => {
      const response = await frappe.call({
        method: "calco_erp.calco_production.work_order_lifecycle.make_work_order_quality_inspection",
        args: { work_order: frm.doc.name, qc_stage: "Initial QC" },
        freeze: true,
      });
      frappe.model.sync(response.message);
      frappe.set_route("Form", "Quality Inspection", response.message.name || "new-quality-inspection");
    }, __("Production QC"));

    frm.add_custom_button(__("Create Final QC"), async () => {
      const response = await frappe.call({
        method: "calco_erp.calco_production.work_order_lifecycle.make_work_order_quality_inspection",
        args: { work_order: frm.doc.name, qc_stage: "Final QC" },
        freeze: true,
      });
      frappe.model.sync(response.message);
      frappe.set_route("Form", "Quality Inspection", response.message.name || "new-quality-inspection");
    ...
    frm.add_custom_button(__("Mark Retest Required"), async () => {
      await frappe.call({
        method: "calco_erp.calco_production.work_order_lifecycle.mark_retest_required_for_work_order",
        args: { work_order: frm.doc.name },
        freeze: true,
      });
      await frm.reload_doc();
      frappe.show_alert({ message: __("Retest required."), indicator: "orange" });
    }, __("Production QC"));
  }
  function register(doctype) {
    frappe.ui.form.on(doctype, {
        ...
        wrapper.html(`
          <section class="calco-production-journey">
            <div class="calco-production-journey__header">
              <div>
                <div class="calco-production-journey__title">${__("Production Execution Journey")}</div>
                <div class="calco-production-journey__subtitle">${__("Save then open to load the Production Execution Journey.")}</div>
              </div>
            </div>
          </section>
        `);
      ...
      refresh(frm) {
        loadTracker(frm);
        addWorkOrderLifecycleButtons(frm);
      },
    });
  }
