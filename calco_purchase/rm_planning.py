...
def get_dashboard_data(*args, **kwargs) -> dict[str, object]:
    ...
    return {
        ...
        "formulas": {
            ...
            "suggested_order_qty": "MAX(0, Maximum Level + Production Requirement - Current RM Store Stock - Open PO / In Transit Qty - Open MR Qty)",
        },
    }
...
def get_open_material_request_map(item_codes: list[str]) -> dict[str, float]:
    ...
    rows = frappe.db.sql(
        ...
        select
            mri.item_code,
            round(sum(greatest(mri.qty - ifnull(mri.ordered_qty, 0), 0)), 3) as pending_qty
        ...
        where mr.docstatus = 1
          ...
          and greatest(mri.qty - ifnull(mri.ordered_qty, 0), 0) > 0
...
def build_dashboard_row(
    ...
    maximum_level = manual_maximum_level if manual_maximum_level > 0 else calculated_maximum_level
    ...
    suggested_order_qty = max(
        0,
        maximum_level + production_requirement - current_stock - open_po_qty - open_material_request_qty,
    )
