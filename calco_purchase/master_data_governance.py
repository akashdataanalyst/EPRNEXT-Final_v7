...
frappe.ui.form.on("{dt}", {
    async refresh(frm) {
        ...
        root.find(".calco-governance-journey-host").remove();
        ...
        if (governance && typeof governance.loadTracker === "function") {
            await governance.loadTracker(frm);
        }
    },
    async onload_post_render(frm) {
        ...
        if (governance && typeof governance.loadTracker === "function" && !frm.__governance_journey_data) {
            await governance.loadTracker(frm);
        }
    },
    status(frm) {
